...
class RedisChannelLayer(BaseChannelLayer):
    ...
    async def _brpop_with_clean(self, index, channel, timeout):
        ...
        # Mover los elementos de la cola de respaldo a la cola principal
        backup_elements = await connection.zrange(backup_queue, 0, -1, withscores=True)
        for member, timestamp in backup_elements:
            await connection.zadd(channel, {member: float(timestamp)})
        await connection.delete(backup_queue)
        
        # Obtener elementos de la cola principal
        result = await connection.zrange(channel, 0, 0, withscores=True)
        
        if result:
            member, timestamp = result[0]
            await connection.zrem(channel, member)
    ...
    async def _clean_receive_backup(self, index, channel):
        ...
        # Obtener los elementos más antiguos de la cola de respaldo
        backup_elements = await connection.zrange(backup_queue, 0, 0, withscores=True)
        
        if backup_elements:
            member, _ = backup_elements[0]
            await connection.zrem(backup_queue, member)
